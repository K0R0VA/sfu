...
use crate::{PacketStream, actor::{Actor, Addr}, error::Error, quality_monitor::QualityMonitor, room::StreamQuality, user::handle_rtp_packets};

pub struct VideoSubscription {
    pub pc: Arc<RTCPeerConnection>,
    pub peer_id: Uuid,
    pub connections: HashMap<StreamQuality, PacketStream>,
...
impl VideoSubscription {
    pub async fn new(pc: Arc<RTCPeerConnection>, peer_id: Uuid, mime_type: String, stream: PacketStream, quality: StreamQuality) -> Result<Self, Error> {
        ...
        let active_track = pc.add_transceiver_from_track(
            ...
            .sender()
            .await;
        connections.insert(quality, stream.resubscribe());
...
impl VideoSubscription {
    async fn activate_stalled_connection(&mut self, mut stream: PacketStream) -> Result<(), Error> {
...
pub enum VideoSubscriptionMessage {
    AddSubsription { quality: StreamQuality, stream: PacketStream },
    ...
    async fn handle(&mut self, ctx: &mut crate::actor::Ctx<'_, Self>, m: Self::Message) {
        match m {
            ...
            VideoSubscriptionMessage::SwitchQualityLayer { to  } => {
                ...
                if let Err(e) = self.activate_stalled_connection(stream.resubscribe()).await {
